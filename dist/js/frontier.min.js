...
function handleError(err){alert(err)}function base58checkEncode(x,vbyte){vbyte=vbyte||0;var front=[vbyte].concat(Bitcoin.convert.hexToBytes(x)),checksum=Bitcoin.Crypto.SHA256(Bitcoin.Crypto.SHA256(front,{asBytes:!0}),{asBytes:!0}).slice(0,4);return Bitcoin.base58.encode(front.concat(checksum))}function getBalanceByDate(value,date){var delta=dhms(1e3*(date-1406066400)),price=maxEthForBtc;return delta.days<0?0:(delta.days>=decreaseAfter&&(price=maxEthForBtc-(delta.days-decreaseAfter+1)*decreaseAmountPerDay),price=Math.max(price,minEthForBtc),value*price)}function dhms(t){var cd=864e5,ch=36e5,cm=6e4,d=Math.floor(t/cd),h=Math.floor((t-d*cd)/ch),m=Math.floor((t-d*cd-h*ch)/cm);return{days:d,hours:h,minutes:m,seconds:Math.round((t-d*cd-h*ch-m*cm)/1e3)}}function shuffle(elems){allElems=function(){for(var ret=[],l=elems.length;l--;)ret[ret.length]=elems[l];return ret}();for(var shuffled=function(){for(var l=allElems.length,ret=[];l--;){var random=Math.floor(Math.random()*allElems.length),randEl=allElems[random].cloneNode(!0);allElems.splice(random,1),ret[ret.length]=randEl}return ret}(),l=elems.length;l--;)elems[l].parentNode.insertBefore(shuffled[l],elems[l].nextSibling),elems[l].parentNode.removeChild(elems[l])}function GetLatestReleaseInfo(){$.getJSON("https://api.github.com/repos/ethereum/mist/releases/latest").done(function(release){for(var downloadCount=(release.assets[0],0),i=0;i<release.assets.length;i++)downloadCount+=release.assets[i].download_count;var releaseInfo=release.name+" and downloaded "+downloadCount.toLocaleString()+" times.";console.log(release,releaseInfo)})}function selectElementContents(el){if(window.getSelection&&document.createRange){var sel=window.getSelection(),range=document.createRange();range.selectNodeContents(el),sel.removeAllRanges(),sel.addRange(range)}else if(document.selection&&document.body.createTextRange){var textRange=document.body.createTextRange();textRange.moveToElementText(el),textRange.select()}}var blockchainUrl="https://blockchain.info",blockchainApi="5b846ae8-eb56-4c14-aae9-bd13056b6df7",fundraisingAddr="36PrZ1KHYMpqSyAQXSG8VwbUiq2EogxLo2",satoshisInBtc=1e8,maxEthForBtc=2e3,minEthForBtc=1337.07714935,decreaseAfter=14,decreaseAmountPerDay=30;!function(){var $checkForm=$("#ether-checker");$checkForm.submit(function(e){e.preventDefault();var addr=$checkForm.find("input[name=address]").val();if(addr){var btcaddr=base58checkEncode(addr,0),getUnspentUrl=blockchainUrl+"/unspent?active="+btcaddr+"&cors=true&api_code="+blockchainApi;$.ajax({type:"GET",url:getUnspentUrl,crossDomain:!0,error:function(err){handleError(err.responseText)},success:function(json){function tick(){++counter===unspent.length&&alert(numeral(totalEth).format("0,0.00")+"ETH")}try{var unspent=json.unspent_outputs,totalBtc=0,totalEth=0,counter=0;unspent.forEach(function(tx){var txUrl=blockchainUrl+"/rawtx/"+tx.tx_index+"?cors=true&api_code="+blockchainApi+"&format=json";void 0!==tx.tx_index?$.getJSON(txUrl,function(data){try{if(data.out[0].addr!==fundraisingAddr)throw new Error(data.out[0].addr+" is not the fundraising address");var btc=(data.out[0].value+3e4)/satoshisInBtc,eth=getBalanceByDate(btc,data.time);totalBtc+=btc,totalEth+=eth,tick()}catch(e){handleError(e)}}):tick()})}catch(e){handleError(e)}}})}})}(),$(document).ready(function(){$("code").each(function(i,block){hljs.highlightBlock(block),$(block).click(function(){selectElementContents(block)})}),$("#legal").change(function(e){$(this).is(":checked")?$("#donate_to").css({display:"block"}):$("#donate_to").css({display:"none"})}),$("#tableOfContents").tableOfContents(".inner-tutorial",{startLevel:"3",depth:"5"}),$(".require-legal").click(function(e){e.preventDefault(),$(".legal-modal").addClass("visible"),console.log(this,$(this)),$(".button-proceed").attr("href",$(this).attr("href"))}),$(".button-cancel, .button-proceed").click(function(){$(".legal-modal").removeClass("visible")});var toc=document.getElementById("tableOfContents");toc&&(window.onscroll=function(){var innerTutorial=document.getElementsByClassName("inner-tutorial"),className="fixed";innerTutorial[0].getBoundingClientRect().top<=40?toc.classList?toc.classList.add(className):toc.className+=" "+className:toc.classList?toc.classList.remove(className):toc.className=toc.className.replace(new RegExp("(^|\\b)"+className.split(" ").join("|")+"(\\b|$)","gi")," ")}),$("body").addClass(window.navigator.platform.substr(0,5))});
